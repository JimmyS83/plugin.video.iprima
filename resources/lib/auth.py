...
def login(email, password, device_id):
	...
	auth_url = s.get('https://auth.iprima.cz/oauth2/authorize?auth_init_url=https%3A%2F%2Fwww.iprima.cz%2F&auth_return_url=https%3A%2F%2Fwww.iprima.cz%2F%3Fauthentication%3Dcancelled&client_id=prima_sso&redirect_uri=https%3A%2F%2Fauth.iprima.cz%2Fsso%2Fauth-check&response_type=code&scope=openid%20email%20profile%20phone%20address%20offline_access&state=prima_sso')
	auth_url = parse_qs(urlparse(auth_url.url).query)
	...
	if get_token.ok:
		profile_id = xbmcplugin.getSetting(plugin.handle, 'profileId')
		if not profile_id:
			data = s.get(url = 'https://auth.iprima.cz/userapi/1/user-profile', headers = {'Authorization' : 'Bearer ' + get_token.json()['access_token']})
			if not data.ok or 'err' in data:
				helpers.displayMessage('Nepodařilo se získat ID profilu', 'ERROR')
				sys.exit()
			else:
				import json
				profiles = json.loads(data.text)
				profile_id = profiles[0]['ulid']
